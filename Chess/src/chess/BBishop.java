...
public class BBishop extends Move {
    ...
    if( !globTurn &&Math.abs(i - getXPos()) == Math.abs(j - getYPos())&&isMovable(i,j,BBishop)&& notPinned(this.i,this.j)&&  isEmpty(i,j) ){

        marker(i,j,BBishop);

       
    }
    else if (!globTurn && Math.abs(i - getXPos()) == Math.abs(j - getYPos())&& isOpp(i,j)&& isMovable(i,j,BBishop)&& notPinned(this.i,this.j)){
